...
// Define Skill interface to type your skills array
interface Skill {
  _id: string;
  name: string;
...
export default function Dashboard() {
  ...
  const [skillType, setSkillType] = useState<"Teaching" | "Learning">(
    "Teaching"
  );
    ...
    const fetchSkills = async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/skills/${user._id}`
        );
  ...
  const handleDelete = async (id: string) => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/skills/delete/${id}`,
        {
          method: "DELETE",
        }
      );
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        if (typeof reader.result === "string") {
          setEditableProfilePic(reader.result);
        }
      };
      ...
      <Tabs defaultValue="overview" className="w-full">
        ...
        <TabsContent value="overview">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
            {skills.map((skill) => (
              <Card key={skill._id} className="p-6 relative">
                <button
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    if (!user?._id) return;
                    const res = await fetch(
                      `${process.env.NEXT_PUBLIC_API_URL}/api/users/update/${user._id}`,
                      {
                        method: "PUT",
                        headers: {
                    ...
                    toast.error(
                      "Network or server error while saving profile."
                    );
                  }
                }}
              >
                ...
                try {
                  ...
                  if (res.ok) {
                    ...
                    if (isEditMode) {
                      setSkills((prev) =>
                        prev.map((skill) =>
                          skill._id === editSkillId ? updatedSkill : skill
                        )
                      );
                    } else {
            ...
            >
              ...
              <div>
                ...
                <select
                  ...
                  value={skillType}
                  onChange={(e) =>
                    setSkillType(e.target.value as "Teaching" | "Learning")
                  }
                  required
                >
